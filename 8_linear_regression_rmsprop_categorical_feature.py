...
class Model(object):
    ...
    def feature(self, degree: int, entry: Entry) -> float:
        if degree < self.degree:
            ...
            return categorical_feature(entry.value)
    ...
    def evolve(self,
        ...
        converge. Therefore, we look to literature and find that AdaGrad, 
        RMSProp and AdaDelta are good methods to force convergence. 
        
        This file implements the RmsProp gradient update algorithm.
        ...
        for i in range(len(self.weights)):
            ...
            self.historical_gradient[i] = self.decay_parameter * (
                self.historical_gradient[i]) + (1 - self.decay_parameter) * (
                gradient ** 2)
