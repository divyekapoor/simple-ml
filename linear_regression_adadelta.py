...
class Model(object):
    def __init__(self, degree):
        ...
        self.evolve_count = 0
        self.historical_gradient = [random.random() for _ in range(0, degree +
                                                                  1)]
        self.historical_delta = [random.random() for _ in range(0, degree + 1)]
    ...
    def evolve(self,
        ...
        for i in range(len(self.weights)):
            ...
            self.historical_gradient[i] = \
                self.decay_parameter * self.historical_gradient[i] + \
                (1 - self.decay_parameter) * (gradient ** 2)
            self.learning_rates[i] = math.sqrt(self.historical_delta[i]) / \
                                     math.sqrt(self.historical_gradient[i])
            gradient_update = self.learning_rates[i] * gradient
            self.historical_delta[i] = \
                self.decay_parameter * self.historical_delta[i] + \
                (1 - self.decay_parameter) * (gradient_update ** 2)
            self.weights[i] -= gradient_update
