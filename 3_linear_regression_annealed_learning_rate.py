...
class Model(object):
    def __init__(self, degree):
        ...
        self.learning_rate = [random.random() for _ in range(0, degree + 1)]
    ...
    def evolve(self,
        ...
        # Update weights
        for i in range(len(self.weights)):
            self.learning_rate[i] = learning_rate[i] / (
                self.epoch_count ** 0.7)
            self.weights[i] -= self.learning_rate[i] * (
    ...
    def __str__(self):
        return 'Model(weights: {}, learning_rate: {}, training_loss: {}, ' \
               'test_loss: {})' \
            .format(self.weights, self.learning_rate, self.training_loss,
                    self.test_loss)
