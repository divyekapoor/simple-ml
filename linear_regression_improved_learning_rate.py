...
class Model(object):
    ...
    def evolve(self,
        ...
        method. The results of this method aren't particularly good.
        ...
        for i in range(len(self.weights)):
            self.learning_rates[i] = learning_rate[i] / (
                2 * (self.feature_cross(i, entry) + regularization_rate))
            self.weights[i] -= self.learning_rates[i] * (
...
if __name__ == '__main__':
    degree = 2
    iterations = 5000
    learning_rate = [0.03, 0.003, 0.0003]
